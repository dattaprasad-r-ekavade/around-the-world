...
| Medium — Resolved 24 September 2026 | **Attachment placement assumed the pose's world matrices were current.** `GltfSkinPose` now tracks dirty matrices and lazily recomputes them when node, mesh, or skin matrix data is read. | `Assets/GltfBoneAttachment.cs`; `Assets/GltfSkinPose.cs`; `GltfAnimationTests.cs` | A regression edits a hand joint and reads the attachment transform without explicitly recomputing; the returned world matrix reflects the edit. |
...
| Low — Resolved 24 September 2026 | `SkinnedMeshGpuBuffer.Draw` accepted any pose with the same joint count, even one from a different skin. | `Assets/SkinnedMeshGpuBuffer.cs`; `GltfAnimationTests.cs` | The buffer now stores its `GltfSkinData` and both draw paths reject poses from another skin. The CPU-side compatibility check covers different skin objects with equal joint counts. |
