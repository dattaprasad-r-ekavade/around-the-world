...
{
    public static StaticMeshData Import(MeshPrimitive primitive)
    {
        ...
        var textureCoordinateAccessor = GetRequiredAccessor(primitive, "TEXCOORD_0");

        var positions = positionAccessor.AsVector3Array();
        var normals = normalAccessor.AsVector3Array();
        var textureCoordinates = textureCoordinateAccessor.AsVector2Array();
        if (normals.Count != positions.Count || textureCoordinates.Count != positions.Count)
            throw new InvalidDataException("POSITION, NORMAL, and TEXCOORD_0 must have the same vertex count.");
        ...
        var indices = primitive.IndexAccessor is null
            ? Enumerable.Range(0, vertices.Length).ToArray()
            : primitive.GetIndices().Select(index => checked((int)index)).ToArray();
